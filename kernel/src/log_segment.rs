...
/// whereas incremental log segments include all actions between two versions.
...
impl LogSegmentBuilder {
    ...
    /// Sets the start version for the log segment. This is optional, and if not set, the
    /// LogSegment will start from 0.
    pub(crate) fn with_start_version(mut self, start_version: Version) -> Self {
    ...
    #[cfg(test)] // currently only used in tests? delete?
    pub(crate) fn with_end_version(mut self, end_version: Version) -> Self {
        self.end_version = Some(end_version);
        self
    }

    /// See [`LogSegmentBuilder::with_end_version`]. This is a convenience method that allows
    /// passing an `Option<Version>` for the end version.
    ...
    #[internal_api]
    pub(crate) fn build_incremental(self, storage: &dyn StorageHandler) -> DeltaResult<LogSegment> {
        require!(
            self.start_version.is_some(),
            Error::generic("TODO: allow building incremental LogSegment without a start version?")
        );
        let start_version = self.start_version.unwrap();
