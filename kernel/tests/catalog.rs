...
mod test_utils {
    ...
    pub async fn put_commit(
        ...
        // Add protocol action for version 0
        if version == 0 {
            commit_lines.push(
                json!({
                    "protocol": {
            ...
            // Add metadata action for version 0
        ...
        info!("putting commit at path: {}", path);
        object_store
        ...
        for add_file in add_files {
            ...
            info!("putting data at path: {}", file_path);
...
#[test]
fn test_log_tail() -> Result<(), Box<dyn std::error::Error>> {
