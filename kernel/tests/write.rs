...
async fn test_commit_info() -> Result<(), Box<dyn std::error::Error>> {
        ...
        setup_test_tables(schema, &[], None, "test_table").await?
    {
        ...
        let snapshot = Arc::new(Snapshot::build(table_url.clone()).build_latest(&engine)?);
...
async fn write_data_and_check_result_and_stats(
    ...
    let snapshot = Arc::new(Snapshot::build(table_url.clone()).build_latest(engine.as_ref())?);
...
async fn test_write_txn_actions() -> Result<(), Box<dyn std::error::Error>> {
        ...
        setup_test_tables(schema, &[], None, "test_table").await?
    {
        ...
        let snapshot = Arc::new(Snapshot::build(table_url.clone()).build_at(1, &engine)?);
        assert_eq!(
